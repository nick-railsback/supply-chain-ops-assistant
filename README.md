...
**Risk is assessed, not assumed.** `propose_action` grades each proposal: one target is `LOW`, 2–10 is `MEDIUM`, more than 10 is `HIGH`, and any irreversible status (`cancelled`, `returned`) forces `HIGH` regardless of count. Anything above `LOW` — or any proposal touching more than five targets — requires confirmation. When the LLM proposes an action it does not get to grade its own risk: the server strips `risk_level` and `requires_confirmation` from the tool schema and recomputes both after the call, so the model can *raise* the risk floor but never lower it.
